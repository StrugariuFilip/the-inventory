...
export default function Products() {
  ...
  const handleAction = async (e) => {
    ...
    if (modalType === 'put') {
      ...
      if (isNameSame && isDescSame && isSkuSame && isCatSame && isPriceSame && isSuppSame) {
        setError("All fields must be modified for a Full Sync ( PUT ) operation.");
        setIsSubmitting(false);
        return;
      }
    }
    ...
    try {
      ...
      if (modalType === 'add') { 
        ...
        actionLabel = "Product added successfully!"; 
        ...
        if (Object.keys(patchData).length === 0) {
          setError("No changes detected for Quick Patch.");
          setIsSubmitting(false);
          return;
        }
        ...
        actionLabel = "Product deleted successfully!"; 
  ...
  const currentFilterName = selectedFilterNode ? warehouses.find(w => w.id.toString() === selectedFilterNode.toString())?.name || 'NODE' : 'DISPLAY ALL';
    ...
    <div className="animate-fade-in space-y-8 relative pb-16 text-white font-sans">
      ...
      {loading ? (
        <div className="w-full py-32 flex flex-col items-center justify-center gap-5">
           ...
           <p className="text-[10px] font-black text-emerald-500/60 uppercase tracking-[0.4em] animate-pulse">Synchronizing Inventory</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="w-full py-20 bg-slate-900/20 border border-slate-800/80 rounded-3xl flex flex-col items-center justify-center gap-3 backdrop-blur-sm"><Package className="h-12 w-12 text-slate-600 stroke-[1.5]" /><p className="text-sm font-black text-slate-500 uppercase tracking-widest font-mono">NO PRODUCTS FOUND</p></div>
      ) : (
            ...
            <div key={p.id} className="group relative bg-slate-900/40 border border-slate-800 rounded-3xl p-6 hover:border-emerald-500/40 transition-all duration-300 backdrop-blur-sm shadow-xl flex flex-col justify-between min-h-[350px] hover:scale-[1.03] hover:shadow-2xl hover:shadow-emerald-500/5">
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-6">
                  <div className="px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-xl font-mono text-[10px] text-emerald-400 uppercase font-black tracking-widest shadow-inner">{p.category || 'ASSET'}</div>  
                  <div className="flex gap-2">
                    <button disabled={isSubmitting} onClick={() => openModal('put', p)} className="group/btn relative p-2.5 bg-slate-950 text-sky-500 hover:bg-sky-500 hover:text-white rounded-xl border border-slate-800 transition-all cursor-pointer"><Edit size={14}/><span className="absolute top-full mt-2 left-1/2 -translate-x-1/2 px-2 py-1 bg-slate-950 text-sky-400 border border-slate-800 rounded text-[9px] font-black uppercase tracking-wider opacity-0 group-hover/btn:opacity-100 transition-all pointer-events-none whitespace-nowrap shadow-xl z-20">FULL SYNC ( PUT )</span></button>
                    <button disabled={isSubmitting} onClick={() => openModal('patch', p)} className="group/btn relative p-2.5 bg-slate-950 text-emerald-500 hover:bg-emerald-500 hover:text-white rounded-xl border border-slate-800 transition-all cursor-pointer"><Zap size={14}/><span className="absolute top-full mt-2 left-1/2 -translate-x-1/2 px-2 py-1 bg-slate-950 text-emerald-400 border border-slate-800 rounded text-[9px] font-black uppercase tracking-wider opacity-0 group-hover/btn:opacity-100 transition-all pointer-events-none whitespace-nowrap shadow-xl z-20">Quick Patch</span></button>
                    <button disabled={isSubmitting} onClick={() => openModal('delete', p)} className="group/btn relative p-2.5 bg-slate-950 text-red-500 hover:bg-red-500 hover:text-white rounded-xl border border-slate-800 transition-all cursor-pointer"><Trash2 size={14}/><span className="absolute top-full mt-2 left-1/2 -translate-x-1/2 px-2 py-1 bg-slate-950 text-red-400 border border-slate-800 rounded text-[9px] font-black uppercase tracking-wider opacity-0 group-hover/btn:opacity-100 transition-all pointer-events-none whitespace-nowrap shadow-xl z-20">Delete</span></button>
                  </div>
                </div>
                ...
                <p className="text-xs text-slate-400/90 italic line-clamp-2 mb-4 h-8 leading-tight font-medium break-words">{p.description || 'No system description provided.'}</p>
                <div className="mb-5 bg-slate-950/60 px-3 py-2 rounded-xl border border-slate-800/60 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 text-[11px] text-slate-300 font-bold truncate"><User size={12} className="text-emerald-500 shrink-0" /><span className="truncate uppercase line-clamp-1 break-all">{suppliers.find(s => s.id === p.supplier_id)?.name || 'UNVERIFIED'}</span></div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800/80"><span className="text-[8px] text-slate-600 block uppercase font-black tracking-widest mb-0.5">Market Value</span><span className="text-lg font-black text-white leading-none">${p.price}</span></div>
                  <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800/80"><span className="text-[8px] text-slate-600 block uppercase font-black tracking-widest mb-0.5">Stock Volume</span><span className="text-lg font-black text-white leading-none flex items-center gap-1"><Box size={12} className="text-emerald-500" /> {p.stockQuantity}</span></div>
                </div>
              </div>
              <div className="relative z-10 pt-5 border-t border-slate-800/50 flex items-center justify-between mt-auto">
                <div className="flex items-center gap-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider truncate"><Warehouse size={12} className="text-emerald-500 shrink-0" /><span className="truncate italic leading-none line-clamp-1 break-all">{warehouses.find(w => w.id === p.warehouse_id)?.name || 'NODE'}</span></div>
          ...
          <div className="relative w-full max-w-xl bg-slate-900 border border-slate-800 rounded-3xl p-7 shadow-2xl z-10 animate-in zoom-in-95 duration-200">
            {modalType === 'delete' ? (
              <div className="text-center space-y-5 py-2">
                <AlertTriangle className="h-12 w-12 text-red-500 mx-auto animate-pulse" /><h2 className="text-2xl font-black text-white uppercase italic tracking-tighter">Confirm Deletion?</h2>
                <div className="flex gap-3 max-w-xs mx-auto pt-2">
                  ...
                  <button disabled={isSubmitting} onClick={handleAction} className="flex-1 py-3 bg-red-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-red-600/20 disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer">{isSubmitting && <Loader2 size={12} className="animate-spin" />}{isSubmitting ? 'DELETING...' : 'DELETE'}</button>
                </div>
              </div>
            ) : (
              <form onSubmit={handleAction} className="space-y-4">
                <div className="flex justify-between items-center pb-3 border-b border-slate-800"><h2 className="text-xl font-black text-white uppercase italic tracking-tight leading-none">{modalType === 'add' ? 'ADD PRODUCT' : modalType === 'put' ? 'FULL SYNC ( PUT )' : 'QUICK PATCH'}</h2><Package size={20} className="text-emerald-500" /></div>
