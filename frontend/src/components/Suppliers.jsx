...
import { 
  ...
  AlertTriangle, CheckCircle2, Mail, Phone, User, ChevronDown
} from 'lucide-react';
...
export default function Suppliers() {
  ...
  const handleAction = async (e) => {
    ...
    try {
      ...
      if (modalType === 'add') { 
        ...
        label = "Supplier deleted successfully!"; 
        tType = 'danger'; 
      }
      
      closeModals(); await fetchSuppliers(); triggerToast(label, tType);
    } catch (err) { 
  ...
  const closeModals = () => {
    setModalType(null); setSelectedSupplier(null); setError('');
    ...
    <div className="animate-fade-in space-y-10 relative pb-16">
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-center justify-between relative z-30">
        <div className="flex items-center space-x-4 w-full lg:w-auto">
          ...
          <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter italic">Supplier Management</h2>
        </div>

        <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full lg:w-auto">
          <div className="relative w-full sm:w-64">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-sky-500" />
            <input type="text" placeholder="FILTER BY ANYTHING.." className="w-full bg-slate-950/90 border border-sky-500/30 rounded-xl py-3 pl-11 pr-4 text-xs text-white font-mono focus:border-sky-500 outline-none transition-all uppercase placeholder:text-slate-600 shadow-[0_0_15px_rgba(14,165,233,0.05)]" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </div>
          <button onClick={() => openModal('add')} className="flex items-center space-x-2 px-5 py-3 rounded-xl bg-sky-500 hover:bg-sky-400 text-slate-950 font-black text-xs transition-all active:scale-95 shadow-md cursor-pointer shrink-0">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filtered.map((s) => (
            <div key={s.id} className="group relative bg-slate-900/30 border border-slate-800/80 rounded-[3rem] p-8 hover:border-sky-500/40 transition-all duration-300 backdrop-blur-sm min-h-[280px] flex flex-col justify-between hover:scale-[1.02]">
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => openModal('put', s)} className="group/btn relative p-2.5 bg-slate-950 text-sky-500 hover:bg-sky-500 hover:text-white rounded-xl border border-slate-800 transition-all cursor-pointer">
                      <Edit size={14}/>
                    ...
                    <button onClick={() => openModal('patch', s)} className="group/btn relative p-2.5 bg-slate-950 text-emerald-500 hover:bg-emerald-500 hover:text-white rounded-xl border border-slate-800 transition-all cursor-pointer">
                      <Zap size={14}/>
                    ...
                    <button onClick={() => openModal('delete', s)} className="group/btn relative p-2.5 bg-slate-950 text-red-500 hover:bg-red-500 hover:text-white rounded-xl border border-slate-800 transition-all cursor-pointer">
                      <Trash2 size={14}/>
                ...
                <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter mb-4 group-hover:text-sky-400 transition-colors truncate">{s.name}</h3>
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-slate-400 font-mono text-[10px] uppercase tracking-widest bg-slate-950/40 px-3 py-1.5 rounded-lg border border-slate-800/50">
                    <Mail size={12} className="text-sky-500 shrink-0" /> <span className="truncate">{s.contact_email}</span>
                  </div>
                  <div className="flex items-center gap-2 text-slate-400 font-mono text-[10px] uppercase tracking-widest bg-slate-950/40 px-3 py-1.5 rounded-lg border border-slate-800/50 w-fit">
            ...
            {modalType === 'delete' ? (
              <div className="text-center space-y-5 py-2">
                ...
                <div className="flex gap-3 max-w-xs mx-auto pt-2">
                  ...
                  <button onClick={handleAction} className="flex-1 py-3 bg-red-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest cursor-pointer">Delete</button>
                </div>
              </div>
            ) : (
              <form onSubmit={handleAction} className="space-y-4">
                ...
                {error && <div className="bg-red-500/10 border border-red-500/20 text-red-500 text-[10px] font-black p-2.5 rounded-lg tracking-widest uppercase">{error}</div>}
                <div className="space-y-3 font-mono">
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-2.5 focus-within:border-sky-500/50 transition-all">
                    <label className="text-[9px] text-slate-500 block mb-1 font-bold">NAME</label>
                    <input type="text" className="w-full bg-transparent border-none text-white font-bold p-0 outline-none text-xs font-sans" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
                  </div>
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-2.5 focus-within:border-sky-500/50 transition-all">
                    <label className="text-[9px] text-slate-500 block mb-1 font-bold">CONTACT_EMAIL</label>
                    <input type="text" className="w-full bg-transparent border-none text-white font-bold p-0 outline-none text-xs font-sans" value={formData.contact_email} onChange={(e) => setFormData({...formData, contact_email: e.target.value})} />
                  </div>
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-2.5 focus-within:border-sky-500/50 transition-all">
                    <label className="text-[9px] text-slate-500 block mb-1 font-bold">PHONE_NUMBER</label>
                    <input type="text" className="w-full bg-transparent border-none text-white font-bold p-0 outline-none text-xs font-sans" value={formData.phone_number} onChange={handlePhoneChange} />
                  </div>
                </div>
                <div className="flex gap-3 pt-3 font-sans">
                   ...
                   <button type="submit" className={`flex-1 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all cursor-pointer shadow-md ${modalType === 'patch' ? 'bg-emerald-500 text-slate-950 hover:bg-emerald-400' : 'bg-sky-500 text-slate-950 hover:bg-sky-400'}`}>CONFIRM</button>
                </div>
              </form>
            )}
          </div>
