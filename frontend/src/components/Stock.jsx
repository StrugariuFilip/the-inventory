...
import { 
  ArrowLeftRight, AlertTriangle, CheckCircle2, ChevronDown, Zap, TrendingUp, TrendingDown
} from 'lucide-react';
...
export default function Stock() {
  ...
  const handleQuickAdd = (amount) => {
    ...
    if (amount === 'MAX') {
      setTransferQuantity(currentProduct.stockQuantity);
      return;
    }
  ...
  const handleAction = async (e) => {
    ...
    if (!sourceWarehouseId || !selectedProductId) {
      setError("Sursa și produsul sunt obligatorii.");
      return;
    }

    if (mode === 'transfer' && !targetWarehouseId) {
      setError("Hub-ul de destinație este obligatoriu pentru executarea transferului.");
      return;
    }

    const qty = parseInt(transferQuantity, 10);
    if (isNaN(qty) || qty <= 0) {
      setError("Cantitatea introdusă trebuie să fie un număr strict pozitiv.");
      return;
    }

    if ((mode === 'transfer' || mode === 'decrease') && qty > currentProduct?.stockQuantity) {
      setError(`Stoc insuficient. Disponibil: ${currentProduct.stockQuantity}`);
      return;
    }

    if (mode === 'transfer' && sourceWarehouseId.toString() === targetWarehouseId.toString()) {
      setError("Nodurile sursă și destinație nu pot fi identice.");
      return;
    }
    ...
    try {
      ...
      if (mode === 'transfer') {
        ...
        successLabel = `TRANSFER COMPLETAT: ${qty}x ${currentProduct.name}`;
      } else if (mode === 'increase') {
        url = `${baseUrl}/increase`;
        payload = { quantity: qty, supplierId: currentProduct.supplier_id };
        successLabel = `STOC MAJORAT: +${qty}x ${currentProduct.name}`;
      } else if (mode === 'decrease') {
        url = `${baseUrl}/decrease`;
        payload = { quantity: qty };
        successLabel = `STOC DEDUS: -${qty}x ${currentProduct.name}`;
      }
      ...
      setError(err.response?.data?.detail || "Tranzacție eșuată. Verifică integritatea bazei de date.");
    }
  };
    ...
    <div className="animate-fade-in space-y-10 relative pb-16 text-slate-100 font-sans">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-30">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-xs font-mono text-indigo-400/80 tracking-widest uppercase mt-1">
              SECURE INTER-FACILITY ROUTING
            </p>
          </div>
        </div>

        <div className="flex items-center bg-slate-950 border border-slate-800/80 p-1.5 rounded-2xl shrink-0 shadow-inner">
          {[
            ...
            return (
              <button
                key={t.id}
                type="button"
                onClick={() => { setMode(t.id); setTransferQuantity(''); setError(''); }}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-black text-[10px] tracking-widest uppercase transition-all cursor-pointer ${
                  active ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 shadow-md' : 'text-slate-500 hover:text-slate-300'
                }`}
              >
      ...
      <form onSubmit={handleAction} className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch pt-2">
        ...
        <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 flex flex-col justify-between relative backdrop-blur-sm shadow-xl">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1 bg-slate-950 border border-slate-800 rounded-full font-mono text-[9px] text-indigo-400 font-black tracking-widest uppercase shadow-md">
            SOURCE HUB
          </div>
          ...
          <div className="space-y-5 mt-2">
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-xl p-3 relative">
              ...
              <button
                type="button"
                onClick={() => setIsSourceWareOpen(!isSourceWareOpen)}
              ...
              {isSourceWareOpen && (
                <>
                  <div className="fixed inset-0 z-30" onClick={() => setIsSourceWareOpen(false)} />
                  <div className="absolute left-0 right-0 mt-2 bg-slate-950 border border-slate-800 rounded-xl shadow-2xl overflow-hidden z-40 max-h-40 overflow-y-auto text-[10px]">
                    {warehouses.map(w => (
                      <div 
                        key={w.id}
                        onClick={() => { 
                          setSourceWarehouseId(w.id); 
                          if(targetWarehouseId === w.id.toString()) setTargetWarehouseId(''); 
                          setIsSourceWareOpen(false); 
                        }}
                        className={`px-3 py-2.5 cursor-pointer flex items-center justify-between border-b border-slate-900/50 last:border-none font-sans ${sourceWarehouseId.toString() === w.id.toString() ? 'bg-indigo-500/10 text-indigo-400 font-bold' : 'text-slate-300 hover:bg-slate-900'}`}
                      >
            ...
            <div className={`bg-slate-950 border border-slate-800 rounded-xl p-3 relative ${!sourceWarehouseId ? 'opacity-50 pointer-events-none' : ''}`}>
              ...
              >
                <span className="truncate">
                  {currentProduct ? `${currentProduct.name} (SKU: ${currentProduct.sku})` : sourceProducts.length === 0 ? 'NO ASSETS AVAILABLE' : 'CHOOSE PRODUCT'}
                </span>
              ...
              {isProductOpen && sourceProducts.length > 0 && (
                <>
                  <div className="fixed inset-0 z-30" onClick={() => setIsProductOpen(false)} />
                  <div className="absolute left-0 right-0 mt-2 bg-slate-950 border border-slate-800 rounded-xl shadow-2xl overflow-hidden z-40 max-h-48 overflow-y-auto text-[10px]">
                    {sourceProducts.map(p => (
                      <div 
                        key={p.id}
                        onClick={() => { setSelectedProductId(p.id); setTransferQuantity(''); setIsProductOpen(false); }}
                        className={`px-3 py-2 cursor-pointer flex justify-between items-center border-b border-slate-900/50 last:border-none font-sans ${selectedProductId.toString() === p.id.toString() ? 'bg-indigo-500/10' : 'hover:bg-slate-900'}`}
                      >
                        <span className="truncate font-bold text-white uppercase">{p.name}</span>
                        <span className="text-[9px] font-mono text-indigo-400 font-bold shrink-0">QTY: {p.stockQuantity}</span>
                      </div>
                    ))}
                  </div>
                </>
          ...
          <div className="mt-6 pt-5 border-t border-slate-800/60 bg-slate-950/60 p-4 rounded-2xl border border-slate-800/40 text-center">
            ...
            <span className={`text-3xl font-black font-mono leading-none ${currentProduct ? 'text-white' : 'text-slate-700'}`}>
              {currentProduct ? currentProduct.stockQuantity : '0'}
            </span>
        ...
        <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 flex flex-col justify-between relative backdrop-blur-sm shadow-xl">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1 bg-slate-950 border border-slate-800 rounded-full font-mono text-[9px] text-emerald-400 font-black tracking-widest uppercase shadow-md">
            TRADE QUANTITY
          </div>
            ...
            <div className="w-full text-center">
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 focus-within:border-emerald-500/50 transition-all max-w-xs mx-auto">
                <input 
                  type="number" 
                  placeholder="0"
                  className="w-full bg-transparent text-center text-4xl font-black font-mono text-white outline-none no-spinner placeholder:text-slate-800"
              ...
              <div className="grid grid-cols-4 gap-2">
                {[1, 5, 10, 'MAX'].map((val) => {
                  ...
                  if (typeof val === 'number') {
                    displayLabel = mode === 'decrease' ? `-${val}` : `+${val}`;
                  }
                  return (
                    <button
                      key={val}
                      ...
                      className="py-2 bg-slate-950 hover:bg-slate-800 border border-slate-800 hover:border-emerald-500/40 rounded-xl font-mono text-xs font-bold text-slate-300 hover:text-white transition-all cursor-pointer active:scale-95"
                    >
                      {displayLabel}
                    </button>
                  );
          ...
          <div className="pt-4 border-t border-slate-800/50 flex items-center justify-center gap-2 text-[10px] font-mono text-slate-500 uppercase">
            <Zap size={12} className="text-emerald-500" />
            <span>INSTANT PIPELINE ROUTING</span>
          </div>
        </div>
              
        <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 flex flex-col justify-between relative backdrop-blur-sm shadow-xl">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1 bg-slate-950 border border-slate-800 rounded-full font-mono text-[9px] text-amber-500 font-black tracking-widest uppercase shadow-md">
            {mode === 'transfer' ? 'TARGET HUB' : 'EXECUTION HUB'}
          </div>
          ...
          {mode === 'transfer' ? (
            <div className="space-y-5 mt-2">
              <div className="bg-slate-950 border border-slate-800 rounded-xl p-3 relative">
                ...
                <button
                  type="button"
                  onClick={() => setIsTargetWareOpen(!isTargetWareOpen)}
                  className="w-full bg-transparent text-left font-bold text-xs flex items-center justify-between outline-none cursor-pointer text-white uppercase font-sans"
                >
                ...
                {isTargetWareOpen && (
                  <>
                    <div className="fixed inset-0 z-30" onClick={() => setIsTargetWareOpen(false)} />
                    <div className="absolute left-0 right-0 mt-2 bg-slate-950 border border-slate-800 rounded-xl shadow-2xl overflow-hidden z-40 max-h-40 overflow-y-auto text-[10px]">
                      {warehouses.map(w => {
                        const isDisabled = w.id.toString() === sourceWarehouseId.toString(); 
                        return (
                          <div 
                            key={w.id}
                            onClick={() => { 
                              ...
                              setIsTargetWareOpen(false); 
                            }}
                            className={`px-3 py-2.5 flex items-center justify-between border-b border-slate-900/50 last:border-none font-sans ${isDisabled ? 'opacity-30 cursor-not-allowed text-slate-600' : 'cursor-pointer text-slate-300 hover:bg-slate-900'}`}
                          >
                            <span className="truncate">ID: {w.id} — {w.name}</span>
                          </div>
                        );
            ...
            <div className="flex flex-col justify-center h-full space-y-4 my-auto text-center">
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex items-center justify-center gap-4">
                <div className="text-right">
                  <span className="text-[8px] text-slate-600 block uppercase font-mono">CURRENT</span>
                  <span className="text-xl font-black font-mono text-slate-400">{currentProduct ? currentProduct.stockQuantity : 0}</span>
                </div>
                <ArrowLeftRight size={16} className="text-slate-600 shrink-0" />
                <div className="text-left">
                  <span className="text-[8px] text-indigo-400 block uppercase font-mono font-bold">CALCULATED</span>
                  <span className={`text-xl font-black font-mono ${mode === 'increase' ? 'text-emerald-400' : 'text-amber-400'}`}>
                    {calculatedStock}
                  </span>
                </div>
              </div>
              <p className="text-[10px] text-slate-500 italic">
                {mode === 'increase' ? 'Units will be added to source facility.' : 'Units will be deducted permanently.'}
              </p>
            </div>
          )}
          ...
          <div className="mt-auto pt-6">
            <button
              type="submit"
              className={`w-full py-4 text-white font-black text-xs uppercase tracking-[0.2em] rounded-2xl shadow-xl transition-all cursor-pointer active:scale-[0.98] border border-white/10 ${
                mode === 'transfer' 
                  ? 'bg-gradient-to-r from-indigo-600 via-sky-600 to-emerald-600 hover:from-indigo-500 hover:to-emerald-500' 
                  : mode === 'increase'
                  ? 'bg-emerald-600 hover:bg-emerald-500'
                  : 'bg-amber-600 hover:bg-amber-500'
              }`}
            >
              {mode === 'transfer' ? 'EXECUTE TRANSFER' : mode === 'increase' ? 'COMMIT INCREASE' : 'COMMIT DECREASE'}
            </button>
          </div>
